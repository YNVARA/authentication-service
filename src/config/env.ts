...
const rawEnvSchema = z.object({
    ...
    SMTP_USERNAME: z.string().default('user'),
    SMTP_PASSWORD: z.string().default('pass'),
    ...
    JWT_REFRESH_TOKEN_SECRET: z.string().default('refresh-secret-change-this-in-production'),
    ...
    // OTP CONFIGURATION
    OTP_SECRET: z.string().default('supersecret'),
    ...
    SESSION_TTL: z.coerce.number().int().positive().default(3600),
    VERIFICATION_TTL: z.coerce.number().int().positive().default(86400), // Default 24 hours
    PASSWORD_RESET_TTL: z.coerce.number().int().positive().default(3600), // Default 1 hour
    MFA_TTL: z.coerce.number().int().positive().default(600), // Default 10 minutes
...
export const env = {
    ...
    smtp: {
        host: raw.SMTP_HOST,
        port: parseInt(raw.SMTP_PORT, 10),
        username: raw.SMTP_USERNAME,
        password: raw.SMTP_PASSWORD,
    },
    otp: {
        secret: raw.OTP_SECRET,
    },
    jwt: {
        accessTokenSecret: raw.JWT_ACCESS_TOKEN_SECRET,
        accessTokenExpiry: raw.JWT_ACCESS_TOKEN_EXPIRY,
        refreshTokenSecret: raw.JWT_REFRESH_TOKEN_SECRET,
        refreshTokenExpiry: raw.JWT_REFRESH_TOKEN_EXPIRY,
    ...
    redis: {
        host: raw.REDIS_HOST,
        port: raw.REDIS_PORT,
        password: raw.REDIS_PASS,
    },
    ttl: {
        session: raw.SESSION_TTL,
        verification: raw.VERIFICATION_TTL,
        passwordReset: raw.PASSWORD_RESET_TTL,
        mfa: raw.MFA_TTL,
    },
    minio: {
